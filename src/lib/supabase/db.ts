...
function mapInvoice(row: Record<string, unknown>): Invoice {
  return normalizeInvoice({
    ...
    monthlyItems: (row.monthly_items as InvoiceLineItem[]) ?? [],
...
function mapQuote(row: Record<string, unknown>): Quote {
  return normalizeQuote({
    ...
    monthlyItems: (row.monthly_items as QuoteLineItem[]) ?? [],
    currency: "EUR",
    subtotal: Number(row.subtotal ?? 0),
    total: Number(row.total ?? 0),
    monthlyTotal: Number(row.monthly_total ?? 0),
  ...
  if (invoices.length) {
    const { error } = await supabase.from("invoices").upsert(
      invoices.map((i) => ({
        ...
        monthly_items: i.monthlyItems,
        subtotal: i.subtotal,
        tax_total: i.taxTotal,
        total: i.total,
        monthly_total: i.monthlyTotal,
        notes: i.notes,
