  ...
  for (const block of html.matchAll(
    ...
    try {
      ...
      for (const node of nodes) {
        ...
        if (/Organization|LocalBusiness|Corporation/i.test(t) && typeof o.name === "string") {
          const n = o.name.trim();
          if (n) return n;
        }
      }
...
export async function fetchSite(url: string): Promise<QualifySiteResult> {
  ...
  try {
    ...
    const description =
      ...
      null;
    const companyNameHint = companyNameHintFrom($, title, html);
    ...
    const phones = [
      ...
      .slice(0, 6);
    return { title, description, companyNameHint, excerpt, emails, phones };
  } catch (e) {
